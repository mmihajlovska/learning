			...
			model = lSub ? JSON.parse(lSub) : {users:[{ime:'Marina', prezime:'Mihajlovska', data:'15.07.86',tel:'908',rentedMovies:[]}],movies:[{ime:'Rambo',god:'1900', user:''}]};	
			changed = 0;
			...
			function rentMovie(i){
				if(model.movies[i].user == ''){
					...
					model.users[u].rentedMovies.push({name:model.movies[i].ime,dateRent:Date(),dateReturn:''});
				}
				submit();
				refresh(checkModel());
			...
			function showRentedMovies(i){
				...
				for (var j = 0; j < model.users[i].rentedMovies.length; j++) {
					html += '<tr><td>' + model.users[i].rentedMovies[j].name + '</td><td>' + model.users[i].rentedMovies[j].dateRent + '</td><td>' + model.users[i].rentedMovies[j].dateReturn + '</td></tr>';
			...
			function constructionTable(i,pr){
				if(activeUsers){
					var edit = '<button class="btn btn-success" onclick="rename(' + i + ');">Edit</button>';
					var rentedM = '<button class="btn btn-info" onclick="showRentedMovies(' + i + ');">Rented movies</button>';
					var checkbox = '<input type="checkbox" onclick="checkboxClick(' + i + ');">';
					var d = '<button class="btn btn-danger" onclick="del(' + i + ');">Delete</button>';
					return '<tr>' + td(pr.ime) + td(pr.prezime)+ td(pr.data)+ td(pr.tel) + td(checkbox + ' ' + rentedM + ' ' + edit + ' ' + d)+'</tr>';
				}else{
					var edit = '<button class="btn btn-success" onclick="rename(' + i + ');">Edit</button>';
					var rentMovie = '<button class="btn btn-info" onclick="rentMovie(' + i + ');">Rent Movie</button>';
					var d = '<button class="btn btn-danger" onclick="del(' + i + ');">Delete</button>';
					var returnM = '<button class="btn btn-warning" onclick="returnMovies(' + i + ');">Return Movie</button>';
					return '<tr>' + td(pr.ime) + td(pr.god) + td(pr.user)+ td(rentMovie + ' ' + returnM + ' ' + edit + ' '  + d)+'</tr>';
				}
			}
			...
			function dataUser(){
				...
				var korisnik = {ime:ime || '', prezime:prezime || '', data:dataNaRaganje || '',tel:tel || '', rentedMovies:[]};
				return korisnik;
			}
